...
On first creation, the Node.js / Python / Docker-in-Docker / Claude Code features are set up. On every container start, an egress firewall is enabled that only allows traffic to GitHub's published IP ranges plus the domains listed in `.devcontainer/scripts/allowed-domains.txt`.

- To disable the firewall, set `INIT_FIREWALL=false` on the host before opening/rebuilding the container.
