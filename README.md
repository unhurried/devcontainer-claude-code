...
- The domain allowlist itself can be switched off without touching the network topology: set `PROXY_MODE=open` in `.devcontainer/.env` and rebuild (default is `allowlist`). squid then allows any destination, but the dev container still has no route out except through squid — this loosens *which domains* are reachable, not the structural isolation below. `verify-isolation.sh` reads the same variable so it doesn't flag an unlisted host answering as a failure while `open` is set.
